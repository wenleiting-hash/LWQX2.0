...
export default function JdTasks() {
  ...
  const openSetupModal = async () => {
    ...
    try {
      const res = await getSystemConfig('jd_config');
      const config = res?.data || res || {};
      setSetupConfig(config);
      
      let finalApiUrl = config.apiUrl;
      // 如果数据库里存的是旧版地址，强制预设为新版地址
      if (!finalApiUrl || finalApiUrl === 'https://api.zhetaoke.com:10001/api/open_jingdong_dingdanchaxun.ashx') {
        finalApiUrl = 'http://api.zhetaoke.com:20000/api/open_jing_union_openz_order_row_query.ashx';
      }
  ...
  const fetchLogs = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const res = await getCronLogs({ taskId: 'sync_jd_orders', page, pageSize });
  ...
  const handleManualTrigger = async () => {
    try {
      ...
      const res: any = await runCronTask('sync_jd_orders', extraPayload);
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Button
          type="text"
          size="small"
          className="text-[#E4393C] hover:text-[#c73033] hover:bg-[#E4393C]/5"
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div className="p-6 pb-4 flex items-center gap-2">
          <div className="w-1 h-4 bg-[#E4393C] rounded-full"></div>
      ...
      <Modal
        title={
          <div className="flex items-center gap-2">
            <Settings className="w-5 h-5 text-[#E4393C]" />
            <span className="text-lg text-slate-800 font-medium">京东订单调度设置</span>
          </div>
        }
      ...
      >
        <Form form={form} layout="vertical" className="mt-6">
          ...
          <div className="mt-4 p-5 bg-[#FDF0F0] rounded-lg border border-[#FBE3E4]">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-1.5 h-1.5 rounded-full bg-[#E4393C]"></div>
              <span className="text-[#C82A2E] font-medium text-sm">接口参数回显</span>
            </div>
            <Row gutter={16}>
              <Col span={8}>
                <div className="bg-white p-3 rounded shadow-sm border border-slate-100">
                  <div className="text-xs text-[#E4393C] mb-1">京东 APPKEY</div>
                  <div className="text-slate-700 font-mono text-sm truncate">
                    {setupConfig?.jd_appkey || setupConfig?.jdAppKey || '未配置'}
                  </div>
                </div>
              </Col>
              <Col span={8}>
                <div className="bg-white p-3 rounded shadow-sm border border-slate-100">
                  <div className="text-xs text-[#E4393C] mb-1">折淘客 APPKEY</div>
                  <div className="text-slate-700 font-mono text-sm truncate">
                    {setupConfig?.zjk_appkey || setupConfig?.zhetaokeAppKey || '未配置'}
                  </div>
                </div>
              </Col>
              <Col span={8}>
                <div className="bg-white p-3 rounded shadow-sm border border-slate-100">
                  <div className="text-xs text-[#E4393C] mb-1">京东联盟ID</div>
                  <div className="text-slate-700 font-mono text-sm truncate">
                    {setupConfig?.jd_union_id || setupConfig?.jdUnionId || '未配置'}
                  </div>
                </div>
              </Col>
            </Row>
          ...
          <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-100">
            ...
            <Button
              type="primary"
              style={{ backgroundColor: "#E4393C", borderColor: "#E4393C" }}
