...
export default function JdOrders() {
  ...
  const columns = [
    {
      ...
      width: 150,
      render: (text: string) => (
        <Paragraph ellipsis={{ tooltip: true }} copyable={text ? { text } : false} style={{ margin: 0, color: "#E4393C" }} className="font-mono">
          {text}
        </Paragraph>
      )
    },
    ...
    },
    {
      ...
      align: "right" as const,
      render: (text: number) => (
        <span className="text-[#E4393C] font-semibold">¥{(Number(text) || 0).toFixed(2)}</span>
      )
    },
    {
      title: "状态",
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        <h2 className="text-xl font-semibold text-slate-900 mb-1 flex items-center gap-2">
          <ShoppingCart className="w-6 h-6 text-[#E4393C]" />
          京东订单
        </h2>
